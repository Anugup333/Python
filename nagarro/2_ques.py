...
Foldername[3] =’myfirst’   already exists in our system. So Add1 at the 
end of the folder name i.e foldername[3] =”myfirst1″
...
Explanation 0
   ...
   foldername[3]=’first’ is already existing . so add 1 to it and it 
   become first1
...
for i in range(n):
    if f_name[:i+1].count(f_name[i]) > 1:
        print(f_name[i]+f"{f_name[:i+1].count(f_name[i])-1}")
    else:
        print(f_name[i])